...
test.describe("Aggressive Auto-Updates", () => {
  ...
  test("does not show update modal if no update is available", async ({
    page,
  }) => {
  ...
  test("clicking Update on Next Launch sets flag in localStorage and dismisses modal", async ({
    ...
    const flag = await page.evaluate(() =>
      localStorage.getItem("thegai_update_on_next_launch"),
    );
    expect(flag).toBe("true");
  });

  test("startup installation overlay shows when flag is set in localStorage", async ({
    page,
  }) => {
  ...
  test("startup installation gracefully handles failures and clears flag so user is not locked out", async ({
    page,
  }) => {
    ...
    const errorText = page.locator(
      "text=Install failed. Please restart the app manually.",
    );
