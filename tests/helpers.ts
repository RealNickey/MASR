        ...
        invoke: async (cmd: string, args?: any) => {
          ...
          if (cmd === "plugin:event|emit") {
            ...
            if (listeners && listeners.has(event)) {
              const handlerIds = listeners.get(event);
              for (const handlerId of handlerIds) {
                const callback = (window as any).__TAURI_CALLBACKS__.get(
                  handlerId,
                );
                if (callback) {
                  callback({ event, payload, id: handlerId });
          ...
          if (cmd === "get_initial_setup_status") {
            return {
              phase: "ready",
              downloaded: 0,
              total: 0,
              english_model_id: null,
          ...
          if (cmd === "plugin:updater|download") {
            ...
            if (
              channelStr &&
              typeof channelStr === "string" &&
              channelStr.startsWith("__CHANNEL__:")
            ) {
              ...
              if (cb) {
                setTimeout(() => {
                  ...
                  cb({
                    index: 1,
                    message: { event: "Progress", data: { chunkLength: 50 } },
                  });
                  cb({
                    index: 2,
                    message: { event: "Progress", data: { chunkLength: 50 } },
